...
//    }
    public String encrypt(String message) throws Exception{
        ...
        cipher.init(Cipher.ENCRYPT_MODE, aesKey);
        byte[] cleartext = message.getBytes();
        return new String(cipher.doFinal(cleartext));
    }

    public String getEcodedParams() throws IOException {
        return new String(cipher.getParameters().getEncoded());
    }
...
//    }
    ...
    public String decrypt(String params, String message) throws Exception{
        AlgorithmParameters aesParams = AlgorithmParameters.getInstance("AES");
        aesParams.init(params.getBytes());
        Cipher cipher = Cipher.getInstance("AES/CBC/PKCS5Padding");
        aesKey = new SecretKeySpec(sharedSecret, 0, 16, "AES");
        cipher.init(Cipher.DECRYPT_MODE, aesKey, aesParams);
